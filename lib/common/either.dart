...
sealed class Either<L, R> {
  ...
  Either<R, L> swap() =>
      fold(ifLeft: (left) => Right(left), ifRight: (right) => Left(right));
...
extension EitherExtension<L, R> on Either<L, R> {
  Either<L, C> flatMap<C>(Either<L, C> Function(R right) f) {
    return switch (this) {
      Left() => this as Left<L, C>,
      Right(right: var value) => f(value),
    };
  }
}
